...
class PiecewiseLinear(ApodizationMethod):
    ...
    def calc_apodization(self, arr: Transducer, target: Point, params: xa.Dataset, transform:np.ndarray | None=None):
        ...
        # SI). Element.angle_to_point(..., units="m", matrix=matrix) scales the
        # element's local position to meters before left-multiplying matrix, so
        # no further scaling is needed at this call site.
