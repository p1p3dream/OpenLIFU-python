...
class Direct(DelayMethod):
    ...
    def calc_delays(self, arr: Transducer, target: Point, params: xa.Dataset | None=None, transform:np.ndarray | None=None):
        ...
        # SI). Element.distance_to_point(..., units="m", matrix=matrix) scales
        # the element's local position to meters before left-multiplying matrix,
        # so no further scaling is needed at this call site.
