#!/usr/bin/env python3
"""Per-element transmission scatter diagnostic for GU008.
...
For each of the 64 elements we compute:
  ...
  predicted:  T_bulk * T_interface_slab * (1/r)  where
                ...
                geom = 1 / r  (element-to-target distance; approximately
                               constant on a focused hemispherical array)

Normalize observed -> predicted on the OPEN-PATH elements (path_mm == 0,
no_intersection). Those elements should have T_bulk = 1, no impedance
loss, and should lie exactly on y = x after normalization.

Outputs a 3-panel scatter + fit stats.
...
logger = logging.getLogger("per_element_transmission_gu008")
...
PLOT_PATH = Path.home() / "Data/openlifu-validation/results/per_element_transmission_scatter_gu008.png"
...
# -------------------------------------------------------------------
# Ray-skull geometry (copied from incidence_angle_amplitude_gu008.py).
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# Narrowband projection (|c_i|) -- same as coherence_factor_gu008.py.
# -------------------------------------------------------------------
...
def main() -> int:
    t_total = time.time()
    print("=" * 78)
    print("Per-element transmission scatter (observed vs model-predicted) -- GU008")
    print("=" * 78)
    ...
    # ---- Load MRI + labels (same as incidence_angle script) ----
    ...
    # T_bulk = 10**(-alpha * (path_mm/10) / 20)  [amplitude, Beer's law]
    T_bulk = np.power(10.0, -ALPHA_SKULL_DB_PER_CM * (skull_paths / 10.0) / 20.0)
    # For path_mm==0 (no_intersection), T_bulk==1 by construction.
    ...
    # Apply slab loss only to elements that actually intersect the skull.
    T_int = np.where(hit_flags, T_interface_slab, 1.0)

    # Geometric 1/r factor (element-to-target distance in metres).
    dists_m = np.linalg.norm(element_positions_raw - target_pos_raw, axis=1) * scl_to_m
    geom = 1.0 / dists_m
    ...
    if n_open >= 2:
        # scale such that median(a_obs) = median(T_pred) on the open-path set
        ...
        # Fallback: normalize on the top decile of predicted (least-blocked).
    ...
    # Linear fit log_obs = slope * log_pred + intercept
    ...
    # Residual: observed / predicted (linear). log residual = log_obs - log_pred.
    ratio = amp_observed / np.maximum(T_pred, 1e-30)
    log_ratio = np.log10(ratio)

    # Residual after accounting for the prediction (log_obs - (slope*log_pred + intercept)).
    log_resid_fit = log_obs - pred_hat  # in log10 units; for fit-slope=1 this is log_ratio shifted

    # Correlations of residual with path and theta (among skull-hit elements).
    ...
    # Open-path-only R2 (around y = x, since normalization forces their mean ratio to 1).
    if n_open >= 3:
        ...
        # Dispersion of ratio among open-path (should be tiny if the 1/r model is right).
    ...
    print("\n" + "=" * 78)
    print(" Fit stats (log10 observed vs log10 predicted)")
    print("=" * 78)
    ...
    # Panel A: observed vs predicted (log-log), color by path, markers by open/hit.
    axA = axes[0]
    ...
    # Panel B: residual vs path, color by theta.
    axB = axes[1]
    axB.axhline(0.0, color="k", ls="--", lw=1, alpha=0.6)
    # open-path points at path=0 overlay
    sc_b = axB.scatter(
    ...
    # Panel C: residual vs theta, color by path.
    axC = axes[2]
    ...
    fig.suptitle(
        f"GU008 per-element transmission: observed |c_i| vs physics model "
    ...
    # ---- Per-element table ----
    print("\n" + "=" * 78)
    print(" Per-element table (sorted by predicted ascending)")
    print("=" * 78)
    ...
    print(f"  fit slope:                  {slope:.3f}  (1.0 = model scales 1:1 with observed)")
    print(f"  open-path R^2:              {r2_open:.3f}  (open-path should lie on y=x tightly)")
    print(f"  Spearman rho(resid, path):  {corr_path_spear:.3f}  "
          f"(non-zero -> path-dependence the model misses)")
    print(f"  Spearman rho(resid, theta): {corr_theta_spear:.3f}  "
          f"(non-zero -> angle-dependence the model misses, e.g. Fresnel, shear conversion)")

    if r2 > 0.8:
        v = "Model EXPLAINS per-element transmission well. Trust the physics."
    elif r2 > 0.5:
        v = "Model PARTIALLY explains transmission; residuals point at missing mechanism."
    else:
        v = ("Model does NOT explain per-element transmission on GU008. "
             "Bulk-absorption + impedance is not the dominant mechanism.")
    print(f"\n  {v}")
