#!/usr/bin/env python3
"""Compare ALL delay correction methods through real skull.

Runs FOUR delay-correction strategies on a single subject:
  1. Direct geometric delays (no skull correction)
  2. SimulationCorrected with peak_method="first_arrival"
  3. ComplexWeighted (narrowband DFT phase extraction, with geometric anchoring)
  4. SimulationCorrected with peak_method="argmax" (known-broken baseline)
...
Total: 3 reciprocal sims + 4 forward sims + 4 probes = ~11 GPU calls.
Expected runtime: 10-20 minutes on RTX 4090.
"""
...
# ---------------------------------------------------------------------------
# Sparse sensor sim + mask builder (copied from compare_first_arrival_vs_argmax.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
def main():
    ap = argparse.ArgumentParser(description="Compare ALL delay correction methods through skull")
    ...
    ap.add_argument("--skip-argmax", action="store_true",
                    help="Skip argmax method (saves ~10 min since delays are huge)")
    args = ap.parse_args()
    ...
    t_total = time.time()
    print("=" * 72)
    print(f"ALL delay methods comparison | subject={subj}")
    print(f"  MRI:    {mri_path}")
    print(f"  Labels: {label_path}")
    print(f"  Skip argmax: {args.skip_argmax}")
    print("=" * 72)
    ...
    apod_uniform = np.ones(arr.numelements())
    common_kwargs = dict(
        arr=arr, apod=apod_uniform, freq=FREQ_HZ, cycles=CYCLES, amplitude=AMPLITUDE,
        dt=dt, t_end=t_end, cfl=CFL, gpu=True, source_method="point_source",
    )

    # -----------------------------------------------------------------------
    # Compute FOUR delay sets
    # -----------------------------------------------------------------------
    step = 5

    print(f"\n[{step}] Computing geometric delays (Direct)...")
    direct = Direct(c0=C0)
    ...
    print(f"\n[{step}] Computing SimulationCorrected delays (peak_method='first_arrival')...")
    ...
    print(f"\n[{step}] Computing ComplexWeighted delays...")
    cw = ComplexWeighted(c0=C0, cfl=CFL, n_cycles=3, gpu=True)
    t0 = time.time()
    delays_cw, apod_cw = cw.calc_complex_weights(arr, target, sim_params)
    print(f"  Done in {time.time()-t0:.1f}s")
    print(f"  CW delay range: {delays_cw.min()*1e6:.2f} to {delays_cw.max()*1e6:.2f} us")
    print(f"  CW apod range: [{apod_cw.min():.4f}, {apod_cw.max():.4f}], mean={apod_cw.mean():.4f}")
    step += 1

    delays_argmax = None
    if not args.skip_argmax:
        print(f"\n[{step}] Computing SimulationCorrected delays (peak_method='argmax')...")
    ...
    delta_cw_geo = delays_cw - delays_geo
    delta_cw_fa = delays_cw - delays_first
    ...
    print(f"    CW - geometric:           mean={delta_cw_geo.mean()*1e6:+.3f}, "
          f"std={delta_cw_geo.std()*1e6:.3f}, max|delta|={np.abs(delta_cw_geo).max()*1e6:.3f}")
    print(f"    CW - first_arrival:       mean={delta_cw_fa.mean()*1e6:+.3f}, "
          f"std={delta_cw_fa.std()*1e6:.3f}, max|delta|={np.abs(delta_cw_fa).max()*1e6:.3f}")
    if delays_argmax is not None:
    ...
    # -----------------------------------------------------------------------
    # Run forward sims + probes
    # -----------------------------------------------------------------------
    methods = [
        ("geometric",      delays_geo,   apod_uniform),
        ("first_arrival",  delays_first, apod_uniform),
        ("CW",             delays_cw,    apod_uniform),
        ("CW+apod",        delays_cw,    apod_cw),
    ]
    if delays_argmax is not None:
        methods.append(("argmax", delays_argmax, apod_uniform))

    forward_results = {}
    for i, (label, delays, apod) in enumerate(methods):
        print(f"\n[{step}] Forward sim: {label}...")
        step += 1
        t0 = time.time()
        fwd_kwargs = dict(common_kwargs)
        fwd_kwargs["apod"] = apod
        with gpu_flock():
            result = run_simulation(params=sim_params, delays=delays,
                                    ref_values_only=False, **fwd_kwargs)
        elapsed = time.time() - t0
    ...
    # -----------------------------------------------------------------------
    print("\n" + "=" * 72)
    print(f"RESULTS: ALL delay methods comparison | subject={subj}")
    print("=" * 72)
    ...
    print(f"\n  {'Method':<20s} {'p_focal (Pa)':>14s} {'ratio vs geo':>14s} {'dB vs geo':>12s}")
    print(f"  {'-'*20} {'-'*14} {'-'*14} {'-'*12}")
    for label in [m[0] for m in methods]:
        p = probe_results[label]["p_focal_window_Pa"]
        r = _ratio(p, p_geo)
        db = _db(p, p_geo)
        print(f"  {label:<20s} {p:>14.6f} {r:>14.4f} {db:>+12.2f}")

    # CW vs first_arrival head-to-head
    p_fa = probe_results["first_arrival"]["p_focal_window_Pa"]
    p_cw = probe_results["CW"]["p_focal_window_Pa"]
    p_cw_apod = probe_results["CW+apod"]["p_focal_window_Pa"]
    print(f"\n  Head-to-head:")
    print(f"    CW vs first_arrival:       {_ratio(p_cw, p_fa):.4f}x ({_db(p_cw, p_fa):+.2f} dB)")
    print(f"    CW+apod vs first_arrival:  {_ratio(p_cw_apod, p_fa):.4f}x ({_db(p_cw_apod, p_fa):+.2f} dB)")
    print(f"    CW+apod vs CW:             {_ratio(p_cw_apod, p_cw):.4f}x ({_db(p_cw_apod, p_cw):+.2f} dB)")

    # Forward sim p@target
    ...
    print(
        f"\nMETHOD_COMPARE subject={subj} "
        f"p_geo={p_geo:.6g} "
        f"p_fa={p_fa:.6g} "
        f"p_cw={p_cw:.6g} "
        f"p_cw_apod={p_cw_apod:.6g} "
        f"ratio_fa={_ratio(p_fa, p_geo):.6f} "
        f"ratio_cw={_ratio(p_cw, p_geo):.6f} "
        f"ratio_cw_apod={_ratio(p_cw_apod, p_geo):.6f} "
        f"db_fa={_db(p_fa, p_geo):+.3f} "
        f"db_cw={_db(p_cw, p_geo):+.3f} "
        f"db_cw_apod={_db(p_cw_apod, p_geo):+.3f} "
        f"cw_vs_fa={_ratio(p_cw, p_fa):.6f} "
        f"cw_apod_vs_fa={_ratio(p_cw_apod, p_fa):.6f} "
