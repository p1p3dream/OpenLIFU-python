#!/usr/bin/env python3
"""GLADYS Simulation with nnU-Net segmentation (kwave_array source variant).

Forked from run_gladys_nnunet.py with source_method="kwave_array" instead of
"point_source" to test whether kWaveArray BLI with per-element directivity
suppresses the Fresnel near-field constructive lobes at the aperture that
dominated the raw argmax in the point_source runs.

Label NIfTI mapping (Dataset002 fullhead; see
...
Outputs:
  - pmax NIfTIs for sims A/B/C: ~/Data/openlifu-validation/results/gladys_nnunet_kwavearray_{corrected,geometric,water}_pmax.nii.gz
  - analysis PNG: ~/Data/openlifu-validation/results/gladys_nnunet_kwavearray_pmax_analysis_2026-04-18.png
"""
...
def main():
    ...
    print("  VARIANT: source_method='kwave_array' (BLI + per-element directivity)")
    print("=" * 72)
    ...
    common_kwargs = dict(
        ...
        dt=dt, t_end=t_end, cfl=CFL, gpu=True, source_method="kwave_array",
    )
    ...
    for sim_label, result in [("corrected", result_a), ("geometric", result_b), ("water", result_c)]:
        ...
        out_path = results_dir / f"gladys_nnunet_kwavearray_{sim_label}_pmax.nii.gz"
    ...
    plot_path = results_dir / "gladys_nnunet_kwavearray_pmax_analysis_2026-04-18.png"
